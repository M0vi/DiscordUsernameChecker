...
create_empty_file("names_to_check.txt")
...
with open("names_to_check.txt", "r", encoding='utf-8') as f:
...
if len(config_str) == 2 or os.path.getsize("config.json") == 0 or config.get("remove_proxies") is None:
    ...
    if config.get("remove_proxies") is None:
        ...
        if ask_proxy.lower() in confirmators:
            ...
            if ask_proxy.lower() in confirmators:
                ...
                print(f"Please input proxies to proxies.txt")
...
if len(combos) == 0:
    ...
    with open("names_to_check.txt", "w", encoding='utf-8') as f:
